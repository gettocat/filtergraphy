...
it('should create new media, follow to it, and media must broadcast to followers', function (done) {
    ...
    app.on('getAllFollowedMediaKeys', (callback) => {
        callback(Object.keys(mediaNetInfo))
    })
    ...
    app.on('msg', (msg) => {
        assert(dialogsWithMedia[msg.meta.from].localkey == msg.dialog.localkey);
        done()
    });
    ...
    let _tempkey;
    app.createMedia('broadcasttest', 'MEDIA_PUBLIC')
        .then(() => {
            return app.getMedia('broadcasttest')
        })
        .then(media => {

            return app.follow('broadcasttest')

        })
        .then((e) => {
            //todo, can check sign and hash.
            _tempkey = dialogsWithMedia['broadcasttest'].localkey;
            assert(dialogsWithMedia['broadcasttest'] && mediaNetFollowStateInfo['broadcasttest'][dialogsWithMedia['broadcasttest'].localkey]);
            return app.mediaBroadcast('broadcasttest', Buffer.from('hello'));
        })
        .catch(done)
