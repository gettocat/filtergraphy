...
it('should create new private media and follow', function (done) {
    ...
    app.on('getMediaKeys', (callback) => {
        ...
        for (let i in mediaLocalInfo) {
            arr.push(mediaNetInfo[i].publicKey)
        }
    ...
    app.on('follower', (mediaData, followerkey) => {
        console.log('new follower', followerkey)
        assert(dialogsWithMedia['followtest'].localkey == followerkey);
        done()
    })

    app.on('notfollower', (mediaData, followerKey) => {
        console.log('not follower', followerKey)
    })

    app.on('NET:sendmempool', (data, callback) => {
        let m = data;
        m.message = data.payload;
        app.incomingmessagehandle({
            hash: data.hash,
            message: m
        });
        callback()
    ...
    app.createMedia('followtest', 'MEDIA_PRIVATE')
        .then(() => {
        ...
        .then((e) => {
            ...
            assert(dialogsWithMedia['followtest']);
            return app.follow('followtest')
        })
        /*.then(() => {
            console.log('localkey', dialogsWithMedia['followtest'].localkey)
            for (let i in storage) {
                if (storage[i].publicKey == dialogsWithMedia['followtest'].localkey) {
                    console.log('keystore', storage[i]);
                    break;
                }
            }

            console.log('dialogwith media', dialogsWithMedia['followtest'])
        console.log('media', mediaLocalInfo['followtest'])
        })*/
        .catch(e => {
            assert(e == 'You are already followed to @followtest')
            //done();
        })
        .catch(done)
