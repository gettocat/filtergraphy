...
it('should create new media @testmedia', function (done) {
    ...
    app.on('getMediaInfo', (mediaName, callback) => {
        callback()
    })
    /**
     * another events
    ...
    app.createMedia('test', 'MEDIA_PUBLIC')
        .then(() => {
            return app.getMedia('test')
        })
        .then(media => {

            //console.log(media.type == 'MEDIA_PUBLIC', media.xpub, media.publicKey)
            assert(media.type == 'MEDIA_PUBLIC' && media.xpub && media.publicKey);
            //done();
        })
        .catch(done)
        .then(() => {
            return app.createMedia('test', 'MEDIA_PUBLIC')//try create second time
        })
        .catch(e => {
            assert(e == "Name test already exist")
            done();
        })
