...
it('should create new private media, follow and send message to follower, follower must decrypt message @testmedia', function (done) {
    ...
    app.on('follower', (pubkey, followerKey) => {
        ...
        assert(dialogsWithMedia['broadcasttest'] && dialogsWithMedia['broadcasttest'].localkey == followerKey);
        console.log('--------------------')
    ...
    app.on('msg', (msg) => {
        console.log(msg)
        assert(Buffer.from('hello').equals(msg.content))
        done()
    });
    ...
    app.createMedia('broadcasttest', 'MEDIA_PRIVATE')
        .then(() => {
